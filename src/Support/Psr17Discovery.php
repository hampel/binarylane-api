 ...
 *
 * The package requires the PSR-17 interfaces and no implementation, on purpose - it does not
 * care which HTTP library an application uses. But a consumer who does not care either
 ...
 *
 * By class NAME, then checked with instanceof, rather than by referencing the classes. That
 * is what keeps the package's declared dependencies honest: composer-require-checker sees no
 * undeclared symbol because none is written, and the analysis run with the dev dependencies
 * removed has nothing to miss. It also means a class that exists but is not a factory - or a
 * factory whose package has changed its interfaces - is skipped rather than returned.
 */
final class Psr17Discovery
{
    /**
     * Request-factory class and stream-factory class, per implementation. Guzzle and Nyholm
     * each ship one class doing both roles; Diactoros splits them.
     *
    ...
    /**
     * The same search over a given list, so the not-found path can be exercised on a machine
     * that has Guzzle installed - which is every machine this package is developed on.
     *
